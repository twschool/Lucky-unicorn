...
def show_instructions():
    ...
    print()
    print("Then pressed <enter> to play. You will get a"
          "random token which might "
...
def token_generator(money):
    ...
    while loop is True:
        ...
        # I wrote my own code for this so that is why
        # it's not a random number from 0-100 choosing
        token = ["Horse", "Horse", "Horse", "Horse", "Horse",
                 "Donkey", "Donkey", "Donkey", "Donkey", "Donkey",
                 "Zebra", "Zebra", "Zebra", "Zebra", "Zebra",
                 "Unicorn"]
        # token_choice is the variable that
        # the random token choice is stored inside
        token_choice = random.choice(token)
        # if the token is a unicorn then 4$ is added to the users balance
        if token_choice == "Unicorn":
            money = money + 4
            print(formatter("!", f"Congratulations, you got a"
                            f" unicorn, Your balance is now ${money:.2f}"))
            print()
        # if the token is a horse or a donkey 50c is added to the users balance
        elif token_choice == "Horse":
            money -= 0.5
            print(formatter("H", f"You got a horse,"
                            f" Your balance is now ${money:.2f}"))
            print()
        elif token_choice == "Zebra":
            money -= 0.5
            print(formatter("Z", f"You got a Zebra,"
                            f" Your balance is now ${money:.2f}"))
            print()
        ...
        # if the token is a zebra 1$ is taken away from the user
            ...
            money = money - 1
            print(formatter("D", f"Bad luck, you got a donkey,"
                            f" Your balance is now ${money:.2f}"))
            print()
        if money <= 0:
            # if the user runs out of money then the
            # function will return the bool value
        ...
        if quitting == "X":
            ...
            print(f"You started with ${starting_money:.2f}\n"
                  f" and leave with ${money:.2f}")
            # if the user decides to optionally quit
            # with the money they currently have
            # the function will return the value false
            # as it is False that the user ran out of money
            return False
        else:
            print("Continuing")
...
while looping[0] is not False:
    ...
    # This code runs the answer through a function
    # to determine if the answer is a
    # Valid answer (Either 'yes' or 'no')
    looping = yes_no_func(response_)
    played_before = looping[1]
if played_before == "No":
    # If the user has not played before
    # then this code will call the function
    # That prints the instructions
    show_instructions()
user_balance_ = number_checker("How much"
                               " would you like to play with? $", 1, 10)
# Tells the user how much money they are
# playing with based on the input they put in
print(f"You are playing with ${user_balance_}")
# Force quit is a variable that is set to
# True when the user runs out of money and set to False
# when the user decides to optionally quit the game
